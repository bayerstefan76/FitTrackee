...
class FitFile:
    ...
    def get_max_speed(self, session_num: Union[int, None] = None) -> Union[float, None]:
        ...
        for session_idx in session_range:
            ...
            if 'enhanced_max_speed' in session.keys():
                enhanced = True
                not_there = False
                if session['enhanced_avg_speed'] > max_speed:
    ...
    def get_moving_data(self, session_num: Union[int, None] = None) -> Optional[MovingData]:
        ...
        if num_sessions == 0:
            ...
            return MovingData(elapsed_time, timer_duration, moving_duration, stop_duration, total_distance, avg_speed,
                              max_speed)

    def session_get_position_records(self, session_num: int) -> List:
    ...
    def session_to_gpx(self, session_idx: int):
        gpx = gpxpy.gpx.GPX()
...
def get_fit_info(
    ...
    for session_idx in range(segments_nb):
        ...
        for point_idx, point in enumerate(segment_records):
            ...
            if update_map_data:
                map_data.append([point['position_lat'], point['position_long']])
...
def extract_segment_from_fit_file(
        fit_file_name: str, session_id: int
) -> Optional[str]:
    """
